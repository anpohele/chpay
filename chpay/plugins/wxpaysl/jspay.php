...
$DB->exec("update `pre_order` set `buyer`=:openId where `trade_no`=:trade_no", [':openId'=>$openId, ':trade_no'=>TRADE_NO]);

if($conf['blockusers']){
	$blockusers = explode('|',$conf['blockusers']);
	if(in_array($openId, $blockusers))sysmsg('系统异常无法完成付款');
}
...
if($_GET['d']==1){
	...
	$redirect_url='\'/pay/wxpaysl/ok/'.TRADE_NO.'/\'';
}
?>
<!DOCTYPE html>
<html>
