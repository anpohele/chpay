...
//              "merchant_usercode"=>"YS_test",
    ...
    {
        // echo '<pre>';
        // print_r($this->common->param);exit();
        $myParams = array();
        $myParams['charset'] = 'UTF-8';
        $myParams['method'] = 'ysepay.online.qrcodepay';
        // $myParams['partner_id'] = $this->common->param['seller_id'];
        $myParams['partner_id'] = 'APHL'; //主账户，下面的seller是主账户下注册的子账户
        $myParams['sign_type'] = 'RSA';
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
        $myParams['return_url'] = $this->common->param['return_url'];
...
//            "bank_type" => "1902000"  //微信
        ...
        if (curl_errno($ch)) {
            ...
            // echo '<pre>';
            // print_r($response['ysepay_online_qrcodepay_response']['source_qr_code_url']);
            $sign = $response['sign'];
            // echo $sign;
            ...
            if ($this->common->sign_check($sign, $data) == true) {
                // echo "验证签名成功!";
                $url = $response['ysepay_online_qrcodepay_response']['source_qr_code_url'];
                header("Location: $url");
            } else {
                echo '验证签名失败!';
            }
