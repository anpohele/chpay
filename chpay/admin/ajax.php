...
case 'saveChannel':
	if($_POST['action'] == 'add'){
		...
		$sql = "INSERT INTO pre_channel (name, rate, mode, type, plugin, daytop) VALUES ('{$name}', '{$rate}', '{$mode}', {$type}, '{$plugin}', {$daytop})";
		...
		if($row)
			exit('{"code":-1,"msg":"支付通道名称重复"}');
		$sql = "UPDATE pre_channel SET name='{$name}',rate='{$rate}',type='{$type}',plugin='{$plugin}',daytop='{$daytop}',mode='{$mode}' WHERE id='$id'";
...
case 'channelInfo':
	...
	foreach($inputs as $item){
		$a = explode(':',$item);
		if($a[0] == 'appkey' || $a[0] == 'appsecret'){
			$data .= '<div class="form-group"><label>'.$a[1].'：</label><br/><textarea id="'.$a[0].'" name="'.$a[0].'" rows="2" class="form-control" required>'.$row[$a[0]].'</textarea></div>';
		}else{
			$data .= '<div class="form-group"><label>'.$a[1].'：</label><br/><input type="text" id="'.$a[0].'" name="'.$a[0].'" value="'.$row[$a[0]].'" class="form-control" required/></div>';
		}
	}
