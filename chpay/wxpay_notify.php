	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if($data['return_code']=='SUCCESS'){
			if($data['result_code']=='SUCCESS'){
				...
				if($srow['status']==0){
					if($DB->exec("update `pay_order` set `status` ='1' where `trade_no`='{$data['out_trade_no']}'")){
						$DB->exec("update `pay_order` set `endtime` ='$date',`buyer` ='{$data['openid']}' where `trade_no`='{$data['out_trade_no']}'");
						processOrder($srow);
					}
					return true;
				}else{
